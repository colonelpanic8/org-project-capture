...
** prompt-mode
If you would like to be prompted for the org file that should be used to store tasks for any project for which such a file has not already been selected, you can use the org-projectile's prompt mode. What follows is a minimal configuration with this mode enabled:
